...
class headPoseModule(yarp.RFModule):
    ...
    def configure(self, rf):
        ...
        self.head_pose_port.open('/' + self.module_name + '/head_pose:i')           #changed to i
        ...
        # Open video writer
        #self.video_writer = cv2.VideoWriter('output_video.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (self.width_img, self.height_img))
        timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        filename = f'output_video_{timestamp}.avi'
        self.video_writer = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc(*'MJPG'), 10, (self.width_img, self.height_img))
    ...
    def close(self):
        ...
        self.video_writer.release() #close video writer
        ...
        if self.process:
            ...
            if message is not None:
                ...
                for j, kpt_person in enumerate(kpt):
                    ...
                    #self.state = self.head_pose_port.read()  
                ...
                if self.output_img_port.getOutputCount():
                    ...
                    self.video_writer.write(img)        #store video
                    cv2.waitKey(1) & 0xff
